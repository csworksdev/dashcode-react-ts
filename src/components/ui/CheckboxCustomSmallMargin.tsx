...
const CheckboxCustomSmallMargin: React.FC<CheckboxProps> = ({
	id,
	disabled,
	label,
	value,
	...
	return (
		<label className={`flex items-center ${disabled ? " cursor-not-allowed opacity-50" : "cursor-pointer"}`} id={id}>
			<input
				...
				disabled={disabled}
			/>
			<span className={`css-checkbox h-7 w-7 border flex-none border-slate-300 dark:border-slate-800 inline-flex relative transition-all duration-150 ${value ? activeClass + " bg-primary " : "dark:border-slate-600"}`}>
				{value && (
					<img
						src={CheckImage}
						alt=""
...
export default CheckboxCustomSmallMargin;